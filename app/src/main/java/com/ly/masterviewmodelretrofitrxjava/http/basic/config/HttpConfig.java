package com.ly.masterviewmodelretrofitrxjava.http.basic.config;

public class HttpConfig {
    public static final String HTTP_REQUEST_TYPE_KEY = "requestType";

    public static final String KEY = "key";

    //--------天气

    public static final String BASE_URL_WEATHER = "https://op.juhe.cn/";

    public static final String KEY_WEATHER = "afc28ae28c6f1b520dab5d1ed537f6c0";

    public static final String HTTP_REQUEST_WEATHER = "weather";

    //二维码
    public static final String BASE_URL_QR_CODE = "https://apis.juhe.cn/";

    public static final String KEY_QR_CODE = "527d0174f4269329f38003575b27a769";

    public static final String HTTP_REQUEST_QR_CODE = "qrCode";

    //新闻
    public static final String BASE_URL_NEWS = "https://v.juhe.cn/";

    public static final String KEY_NEWS = "c3f9d6c4c70559205cab02fb9f8d4a66";

    public static final String HTTP_REQUEST_NEWS = "news";
}
